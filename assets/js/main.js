// Importações do Firebase SDK
        ...
        const firebaseConfig = {
            apiKey: "AIzaSyByZBsF4z_ln-663LCYB_BWNuvSV1Icbng",
            authDomain: "controle-de-estoque-castilho.firebaseapp.com",
            projectId: "controle-de-estoque-castilho",
            storageBucket: "controle-de-estoque-castilho.firebasestorage.app",
            messagingSenderId: "1005627014534",
            appId: "1:1005627014534:web:b4058e0482da7d96c90007",
            measurementId: "G-DCQL7NLHLT"
        };
        ...
        let userCustomPermissions = {}; // Permissões granulares vindas do Firestore
        ...
        // Administrador master fixo (sem depender do Firestore)
        const ADMIN_UID = 'diego';
        ...
        const PERMISSIONS = {
            admin: ['create', 'read', 'update', 'delete', 'export', 'import', 'manage_users', 'settings', 'admin_panel'],
            operador: ['read', 'export'],
        ...
        const isReadOnlyRole = () => userRole === 'visitante' || userRole === 'visualizador' || (userRole === 'operador' && !userCustomPermissions.canEntry && !userCustomPermissions.canEdit && !userCustomPermissions.canDelete && !userCustomPermissions.canReq);

        const hasPermission = (action) => {
            if (!userRole) return false;
            // Admin tem tudo
            if (userRole === 'admin') return true;
            // Para operadores: checar permissões granulares do Firestore
            if (userRole === 'operador') {
                if (action === 'read' || action === 'export') return true;
                if (action === 'create') return userCustomPermissions.canEntry === true;
                if (action === 'update') return userCustomPermissions.canEdit === true;
                if (action === 'delete') return userCustomPermissions.canDelete === true;
                if (action === 'requisition') return userCustomPermissions.canReq === true;
                return false;
            }
            // Para outros roles, usar tabela estática
        ...
        const getUserRole = async (userId) => {
            try {
                ...
                const exists = userDoc && (typeof userDoc.exists === 'function' ? userDoc.exists() : userDoc.exists);
                
                if (exists && userDoc.data()) {
        ...
        const updateUIBasedOnPermissions = () => {
            ...
            // Mostrar/ocultar seção e botão do Painel Admin (só para Diego)
            const navSectionAdmin = document.getElementById('nav-section-admin');
            if (navSectionAdmin) navSectionAdmin.classList.toggle('hidden', !isAdmin);

            // Mostrar/ocultar botão admin no drawer mobile
            ...
            createButtons.forEach(btn => {
                if (btn.id === 'create-requisition-btn') {
                    btn.style.display = hasPermission('requisition') ? '' : 'none';
                } else {
            ...
            navViewButtons.forEach(btn => {
                ...
                if (!viewId) return;
                // admin-view só aparece para admin
            ...
            if (!hasPermission('update') && !readOnlyClickGuardWired) {
                ...
                document.addEventListener('click', (e) => {
                    if (isReadOnlyRole() || !hasPermission('update')) {
                        if (e.target.closest('.edit-btn, .adjust-btn')) {
                            e.stopPropagation();
                            e.preventDefault();
                            showToast('🔒 Você não tem permissão para editar. Contate o administrador.', true);
                        }
                    }
                    if (!hasPermission('delete')) {
                        if (e.target.closest('.delete-btn')) {
                            e.stopPropagation();
                            e.preventDefault();
                            showToast('🔒 Você não tem permissão para excluir. Contate o administrador.', true);
                        }
                    }
                }, true);
            }
            ...
            if (readonly && currentViewId && !READONLY_ALLOWED_VIEWS.has(currentViewId) && currentViewId !== 'admin-view') {
        ...
        const updateAppSettingsUI = (settings) => {
            ...
            if (sidebarTitle) sidebarTitle.textContent = nameToUse;
            const logoContainer = document.getElementById('app-logo');
            if (appSettings.logoUrl) {
                logoContainer.innerHTML = `<img src="${appSettings.logoUrl}" alt="Logo" class="h-8 w-auto object-contain">`;
                logoContainer.className = 'p-1 rounded-xl shadow-sm flex items-center justify-center bg-white border border-slate-200';
            } else {
                logoContainer.innerHTML = `<img src="assets/img/logo-castilho.png" alt="Castilho Logo" class="h-8 w-auto object-contain" onerror="this.outerHTML='<span class=\\'material-symbols-outlined text-red-600\\' style=\\'font-size:22px;\\'>inventory_2</span>';">`;
                logoContainer.className = 'p-1.5 rounded-xl flex items-center justify-center h-11 bg-white border border-slate-200';
                logoContainer.style.boxShadow = '0 4px 12px rgba(229,35,32,0.15)';
            }
        };
        ...
        const initializeAppSession = async (customUser) => {
            ...
            currentObraId = 'castilho';
            if (loginBtn) loginBtn.disabled = false;

            // Determinar base de dados - Consolidada em uma única unidade Castilho
            const obraBase = `/artifacts/${appId}/public/data`;
            ...
            // Coleções legadas do módulo Estrela (desativadas)
            ...
            // Ocultar aba Estoque Estrela (pois o app agora é unificado para Castilho)
            ...
            // Atualizar nome do app no header e sidebar
            const obraDisplayName = 'Castilho';
            ...
            const roleLabels = {
                admin: { text: 'Administrador', color: '#E52320' },
                operador: { text: 'Operador', color: '#dc2626' },
                visualizador: { text: 'Visualizador', color: '#475569' },
                visitante: { text: 'Visitante', color: '#0f172a' }
            };
            const roleInfo = roleLabels[userRole] || roleLabels.operador;

            if (userInfoDiv) userInfoDiv.innerHTML = `
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style="background:#E52320;">${initials}</div>
                    <div class="text-left hidden md:block">
                        <p class="text-sm font-semibold" style="color:#0f172a;">${customUser.displayName}</p>
                        <p class="text-xs font-bold px-2 py-0.5 rounded-full inline-block" style="background:rgba(229,35,32,0.08); color:${roleInfo.color};">${roleInfo.text}</p>
                    </div>
                </div>`;
        ...
        function startCoreListeners() {
            ...
            const obraDefaultName = 'Castilho';
        ...
        const loadGelLogo = () => new Promise(resolve => {
            ...
            img.src = 'assets/img/logo-castilho.png?v=1';
        });
...
btn.style.color = isActive ? '#0066FF' : '#6b7280';
        ...
        // --- Gerenciador de Usuários Locais (Fallback se o Firestore estiver sem permissões) ---
        const getLocalUsers = () => {
            try {
                return JSON.parse(localStorage.getItem('castilho_local_users') || '{}');
            } catch (_) {
                return {};
            }
        };

        const saveLocalUser = (userId, userData) => {
            try {
                const users = getLocalUsers();
                users[userId] = userData;
                localStorage.setItem('castilho_local_users', JSON.stringify(users));
            } catch (e) {
        ...
        const setLoginMode = (mode) => {
            ...
            loginFormUser?.classList.toggle('hidden', !isUser);
            loginFormVisitor?.classList.toggle('hidden', !isVisitor);
            loginFormRegister?.classList.toggle('hidden', !isRegister);
            loginError?.classList.add('hidden');
            const loginSuccess = document.getElementById('login-success');
            if (loginSuccess) loginSuccess.classList.add('hidden');
            // Estilizar botões
            const tabs = [
                { btn: loginModeUserBtn, active: isUser, activeColor: '#E52320' },
                { btn: loginModeVisitorBtn, active: isVisitor, activeColor: '#047857' },
                { btn: document.getElementById('login-mode-register'), active: isRegister, activeColor: '#E52320' }
            ];
            tabs.forEach(({ btn, active, activeColor }) => {
                if (!btn) return;
                btn.style.background = active ? '#fff' : 'transparent';
                btn.style.color = active ? activeColor : '#6b7280';
        ...
        const doVisitorLogin = () => {
            const appUser = {
                uid: 'visitante_castilho',
                displayName: 'Visitante — Castilho',
                role: 'visitante',
                obraId: 'castilho'
            };
        ...
        const doLogin = async () => {
            ...
            try {
                ...
                // 1. Verificar credenciais de administrador da Castilho
                const BUILTIN_USERS = {
                ...
                if (builtin) {
                    ...
                    if (hash === expectedHash) {
                        const appUser = { uid: userId, displayName: builtin.displayName, role: builtin.role, obraId: 'castilho' };
                        localStorage.setItem('appUser', JSON.stringify(appUser));
                ...
                // 2. Verificar no Firestore (ou fallback local) para outros usuários
                let data = null;
                ...
                try {
                    ...
                    console.warn('Firestore inacessível ou sem permissão para ler usuário. Verificando usuários locais:', fsErr);
                }

                // Fallback para usuários cadastrados localmente se não achou no Firestore
                if (!userExists) {
                    const localUsers = getLocalUsers();
                    if (localUsers[userId]) {
                        data = localUsers[userId];
                        userExists = true;
                    }
                }

                if (userExists && data && data.passwordHash && data.passwordHash === hash) {
                    const isApproved = data.aprovado === true;
                    const permissions = data.permissions || {};
                    
                    if (!isApproved) {
                        // Usuário não aprovado: entra como visitante com banner de pendente
                        ...
                        // Usuário aprovado: carregar permissões granulares
                        userCustomPermissions = permissions;
                        const appUser = {
                            uid: userId,
                            displayName: data.displayName || username,
                    ...
                    loginError.textContent = userExists ? 'Senha incorreta.' : 'Usuário não encontrado. Verifique o nome ou crie uma conta na aba Registrar.';
                    loginError.classList.remove('hidden');
                    loginBtn.disabled = false;
                    loginBtn.textContent = 'Entrar';
                }
                ...
                loginError.textContent = 'Erro ao fazer login. Verifique seus dados e tente novamente.';
                loginError.classList.remove('hidden');
                loginBtn.disabled = false;
                loginBtn.textContent = 'Entrar';
            }
        ...
        document.getElementById('login-mode-register')?.addEventListener('click', () => setLoginMode('register'));
        ...
        const doRegister = async () => {
            ...
            if (!name || name.length < 2) {
                if (loginError) { loginError.textContent = 'Digite seu nome completo (mínimo 2 caracteres).'; loginError.classList.remove('hidden'); }
                return;
            }
            if (!pwd || pwd.length < 4) {
                if (loginError) { loginError.textContent = 'A senha deve ter no mínimo 4 caracteres.'; loginError.classList.remove('hidden'); }
                return;
            }

            if (registerBtn) { registerBtn.disabled = true; registerBtn.textContent = 'Criando conta...'; }
            ...
            try {
                ...
                const newUserDoc = {
                    ...
                    role: 'operador',
                    aprovado: false,
                    permissions: { canEntry: false, canEdit: false, canDelete: false, canReq: false },
                ...
                try {
                    ...
                    const exists = existing && (typeof existing.exists === 'function' ? existing.exists() : existing.exists);
                    if (exists && existing.data()) {
                        if (loginError) { loginError.textContent = 'Já existe um usuário registrado com esse nome.'; loginError.classList.remove('hidden'); }
                        if (registerBtn) { registerBtn.disabled = false; registerBtn.textContent = 'Criar Conta'; }
                        return;
                    }
                    ...
                    console.warn('Firestore sem permissão de escrita ou inacessível. Salvando usuário no armazenamento local:', fsErr);
                }

                // Sempre salvar no armazenamento local também para garantia total
                saveLocalUser(userId, newUserDoc);

                // Sucesso
                nameInput.value = '';
                pwdInput.value = '';
                if (loginSuccess) {
                    loginSuccess.textContent = `✅ Conta criada com sucesso para "${name}"! Aguarde aprovação do administrador para ter acesso completo.`;
                    loginSuccess.classList.remove('hidden');
                }
                setTimeout(() => setLoginMode('user'), 2500);
            } catch (e) {
                console.error('Erro ao registrar:', e);
                if (loginError) { loginError.textContent = 'Não foi possível registrar a conta. Verifique os dados e tente novamente.'; loginError.classList.remove('hidden'); }
            } finally {
                if (registerBtn) { registerBtn.disabled = false; registerBtn.textContent = 'Criar Conta'; }
            }
        };
        ...
        const renderAdminPanel = async () => {
            ...
            const countTotal = document.getElementById('admin-count-total');
            const countPending = document.getElementById('admin-count-pending');
            const countApproved = document.getElementById('admin-count-approved');
            if (!listEl) return;

            listEl.innerHTML = `<div class="text-center py-12" style="color:#94a3b8;"><span class="material-symbols-outlined" style="font-size:40px;display:block;margin-bottom:8px;">hourglass_empty</span><p class="text-sm font-medium">Carregando usuários...</p></div>`;

            const usersMap = {};

            // 1. Tentar buscar usuários do Firestore
            try {
                const ref = collection(db, `/artifacts/${appId}/public/data/users`);
                const snap = await getDocs(ref);
                snap.forEach(d => { usersMap[d.id] = { id: d.id, ...d.data() }; });
            } catch (fsErr) {
                console.warn('Não foi possível ler usuários do Firestore:', fsErr);
            }

            // 2. Mesclar com usuários salvos localmente
            const localUsers = getLocalUsers();
            Object.keys(localUsers).forEach(uid => {
                if (!usersMap[uid]) {
                    usersMap[uid] = { id: uid, ...localUsers[uid] };
                } else {
                    // Mesclar permissões locais caso tenham sido atualizadas
                    usersMap[uid] = { ...localUsers[uid], ...usersMap[uid] };
                }
            });

            const users = Object.values(usersMap);
            const filtered = users.filter(u => u.id !== 'diego' && u.id !== 'castilho' && u.id !== 'admin');
            ...
            if (filtered.length === 0) {
                listEl.innerHTML = `<div class="text-center py-16" style="color:#94a3b8;"><span class="material-symbols-outlined" style="font-size:56px;display:block;margin-bottom:12px;">group_off</span><p class="text-sm font-medium">Nenhum funcionário registrado ainda.</p><p class="text-xs mt-1" style="color:#cbd5e1;">Quando criarem contas na aba Registrar, eles aparecerão aqui para você aprovar.</p></div>`;
                return;
            }

            // Ordenar: pendentes primeiro
            filtered.sort((a, b) => (a.aprovado === b.aprovado ? 0 : a.aprovado ? 1 : -1));
                ...
                <div class="rounded-2xl border p-4 transition-all" style="background:#fff;border-color:${isApproved ? '#bbf7d0' : '#fde68a'};">
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white flex-shrink-0 text-sm" style="background:${isApproved ? '#16a34a' : '#d97706'};">
                        ...
                        <span class="text-xs font-bold px-2.5 py-1 rounded-full flex-shrink-0" style="background:${isApproved ? 'rgba(22,163,74,0.1)' : 'rgba(217,119,6,0.1)'};color:${isApproved ? '#16a34a' : '#d97706'};">
                            ${isApproved ? '✅ Aprovado' : '⏳ Aguardando'}
                        </span>
                    </div>
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
                        <label class="flex items-center gap-2.5 p-3 rounded-xl cursor-pointer" style="background:#f8fafc;border:1px solid #e2e8f0;">
                            <input type="checkbox" class="perm-check w-4 h-4 accent-red-600" data-uid="${user.id}" data-perm="canEntry" ${perms.canEntry ? 'checked' : ''}>
                            <span class="text-xs font-semibold" style="color:#374151;">
                                <span class="material-symbols-outlined align-middle" style="font-size:16px;color:#E52320;">input</span>
                                Dar Entrada
                            </span>
                        </label>
                        <label class="flex items-center gap-2.5 p-3 rounded-xl cursor-pointer" style="background:#f8fafc;border:1px solid #e2e8f0;">
                            <input type="checkbox" class="perm-check w-4 h-4 accent-red-600" data-uid="${user.id}" data-perm="canEdit" ${perms.canEdit ? 'checked' : ''}>
                            <span class="text-xs font-semibold" style="color:#374151;">
                                <span class="material-symbols-outlined align-middle" style="font-size:16px;color:#E52320;">edit</span>
                                Editar Itens
                            </span>
                        </label>
                        <label class="flex items-center gap-2.5 p-3 rounded-xl cursor-pointer" style="background:#f8fafc;border:1px solid #e2e8f0;">
                            <input type="checkbox" class="perm-check w-4 h-4 accent-red-600" data-uid="${user.id}" data-perm="canDelete" ${perms.canDelete ? 'checked' : ''}>
                            <span class="text-xs font-semibold" style="color:#374151;">
                                <span class="material-symbols-outlined align-middle" style="font-size:16px;color:#E52320;">delete</span>
                                Excluir Itens
                            </span>
                        </label>
                        <label class="flex items-center gap-2.5 p-3 rounded-xl cursor-pointer" style="background:#f8fafc;border:1px solid #e2e8f0;">
                            <input type="checkbox" class="perm-check w-4 h-4 accent-red-600" data-uid="${user.id}" data-perm="canReq" ${perms.canReq ? 'checked' : ''}>
                            <span class="text-xs font-semibold" style="color:#374151;">
                                <span class="material-symbols-outlined align-middle" style="font-size:16px;color:#E52320;">assignment</span>
                                Requisições
                            </span>
                        </label>
                    </div>
        ...
        document.addEventListener('click', async (e) => {
            const btn = e.target.closest('.save-user-perms-btn');
            if (!btn || userRole !== 'admin') return;
            const uid = btn.dataset.uid;
            const checks = document.querySelectorAll('.perm-check[data-uid="' + uid + '"]');
            const perms = {};
            ...
            try {
                ...
                console.warn('Não foi possível atualizar o Firestore:', err);
            }

            showToast(`✅ Permissões de "${uid}" salvas e conta aprovada!`);
            await renderAdminPanel();
        });

        // Botão Atualizar do painel admin
        document.getElementById('admin-refresh-btn')?.addEventListener('click', renderAdminPanel);

        // Carregar painel admin automaticamente quando a view for acessada
        document.querySelectorAll('[data-view="admin-view"]').forEach(btn => {
            btn.addEventListener('click', () => {
